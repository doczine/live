...
<form id="sign-up-form" name="form" action="" method="post" enctype="multipart/form-data">
<h2>Proof of the Intent to Graduate (LETTER) Unprocessed Form Submittals</h2>
<h4>
<p style="width:880px; text-align:left;">These are the intent to graduate statements sent by the student that have not been approved or denied, and still need to be reviewed by the Evaluation Unit with help of the UMA.</p>
</h4>
<h2>
<a href="http://ls-advise.berkeley.edu/fp/24Intent_To_Grad.pdf">Sample Proof of the Intent to Graduate PDF Version</a>
</h2>
<?php

$conn = create_base_mysqli();

$query = "SELECT `student_id`,`student_name`,`email_verify`,`major`,`mailing_address`,`city`,`state`,`zip`,`phone`,`mailing_address_second`,`city_second`,`state_second`,`zip_second`,`pdf_key`,`time`,`dept_cd_text_1`,`course_title_text_1`,`unit_text_1`,`dept_cd_text_2`,`course_title_text_2`,`unit_text_2`,`dept_cd_text_3`,`course_title_text_3`,`unit_text_3`,`dept_cd_text_4`,`course_title_text_4`,`unit_text_4`,`dept_cd_text_5`,`course_title_text_5`,`unit_text_5`,`dept_cd_text_6`,`course_title_text_6`,`unit_text_6`,`dept_cd_text_7`,`course_title_text_7`,`unit_text_7`,`dept_cd_text_8`,`course_title_text_8`,`unit_text_8`,`dept_cd_text_9`,`course_title_text_9`,`unit_text_9`,`dept_cd_text_10`,`course_title_text_10`,`unit_text_10`,`dept_cd_text_11`,`course_title_text_11`,`unit_text_11`,`dept_cd_text_12`,`course_title_text_12`,`unit_text_12`,`dept_cd_text_13`,`course_title_text_13`,`unit_text_13`,`notes` FROM `test`.`test` WHERE `test`.`eval_checkbox` = '';";

echo "<table border ='1'>";
echo "<th>Student ID</th><th>Student Name</th><th>Student Email</th><th>Major</th><th>Student Name</th><th>Time Date Submitted</th><th>Notes</th>";
...
if ($stmt = mysqli_prepare($conn, $query)) {
    ...
    while (mysqli_stmt_fetch($stmt)) {
		echo "<tr>";
		echo "<td>";
		echo "<a href='https://etriever.berkeley.edu/test_eval_form.php?student_id=".$student_id;
		echo "'>student_id=".$student_id."</a></td>"; 
		echo "<td>";
		echo $student_name;echo "</td>"; 
		echo "<td>";
		echo $email_verify;echo "</td>"; 
		echo "<td>";
		echo $major;echo "</td>";
		 echo "<td>";
		echo $pdf_key;echo "</td>"; 
		echo "<td>";
		echo $time;echo "</td>"; 
		echo "<td>";
		echo $notes;echo "</td>"; 
